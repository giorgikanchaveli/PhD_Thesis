...
$$
where $\text{Lip}_1(\X,\R)$ denotes the set of $1$-Lipschitz continuous functions from $\X$ to $\R$. Since $(\probprobx, \WW)$ is a Polish space with bounded diameter, we can use the above result to obtain, for all $Q^1,Q^2\in\probprobx$,
\[
...
By representing both distance functions as Integral Probability Metrics, we see that HIPM is upper bounded by the WoW distance. Indeed, we can show that $\D$ is a subset of $\text{Lip}_1(\probx, \R, \W)$: let $h \in \mathcal{D}$ and $P^1, P^2 \in \probx$; then
\begin{align*}
