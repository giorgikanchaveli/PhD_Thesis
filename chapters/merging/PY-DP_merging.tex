...
Let $\tilde{\mu}$ be a Cox CRM with random L\'evy intensity $\tilde{\nu}$, as in Definition~\ref{definition:Pitman_Yor_cox_crm}. The random L\'evy intensity has the form
\[
...
\[
\mustar \mid U \overset{d}{=} \tilde{\mu}_U + \sum_{i = 1}^k J_i^U \delta_{x_i^*},
\]
where
\begin{itemize}
    \item $\tilde{\mu}_U$ is a generalized Gamma CRM with L\'evy intensity
    \[
...
\]
The next step is to obtain the scaled version of $\mustar$.  Definition~\ref{definition:scaled_cox_crm}, we obtain the scaled version of the latent CRM for fixed $U$.
...
\]
by Lemma~SM3 in the Supplementary Material~\cite{catalano2026merging}. By applying the same argument to 
\[
...
&\quad + \E\!\left[\mustar(\X) \mid U\right] \rho^2_x\!\left(\E\!\left[\mustar(\X) \mid U\right]s\right)\mathrm{d}P^2_0(x).
\end{align*}
...
\begin{proposition}\label{proposition:distance_pitmanyor_gamma}
    ...
    where
    \begin{align*}
        & \mathcal{A}^U = \wtr\left( \frac{\sigma U^\sigma}{C}P_0 + \frac{1}{C}\sum_{i = 1}^{k}(n_i - \sigma)\delta_{x_i^*}, \frac{\alpha}{\alpha + n}P_0 + \frac{1}{\alpha + n}\sum_{i = 1}^n \delta_{x_i}\right), \\
...
\vspace{-\topsep}
    ...
    \begin{align*}
    \wad\left( \law\left(\tilde{\nu}^{*,1}_{S}\right),  \law\left(\tilde{\nu}^{*,2}_{S}\right) \right) &= \E_{U \sim f_U}\!\ad\!\left(\tilde{\nu}^{*,1}_{S}, \tilde{\nu}^{*,2}_{S}\right) \\
    ...
    The first term is
    \[
    \E_{U \sim f_U}\wtr\!\left(\frac{\sigma U^\sigma}{C}P_0 + \frac{1}{C}\sum_{i = 1}^{k}(n_i - \sigma) \delta_{x_i^*},
    ...
    \begin{align*}
        ...
        & +\E_{U \sim f_U}\frac{1}{C}\sum_{i = 1}^{k} (n_i - \sigma)\wext\!\left(\rho_{x_i^*}^{*,1} , \rho^{*,2} \right) \\
...
\begin{proof}
    ...
    In view of Proposition~\ref{proposition:distance_pitmanyor_gamma}, we study the behaviour of each term in 
    \[
    \E_U\left(J_1^{U_n} + J_2^{U_n} + \mathcal{A}^{U_n}\right).
    \]
    \begin{itemize}
    ...
    It follows that
    \[
    \E \mathcal{A}^{U_n} = \E \mathcal{A}^{U_n}\mathbf{1}_{A_n} + \E \mathcal{A}^{U_n}\mathbf{1}_{A^c_n}.
    \]
    On $A_n$, we decompose
    \[
    ...
    \end{align*}
    We conclude that $\E \mathcal{A}^{U_n} \mathbf{1}_{A_n} \lesssim \frac{k_n}{n}.$
    ...
    \begin{align*}
        ...
        & = \frac{n - k_n\sigma}{C_n}\wtr \Bigg( \sum_{i = 1}^{k_n}\frac{n_i - \sigma}{n - k_n\sigma}\delta_{x_i^*},\\
        & \quad \frac{C_n}{n - k_n\sigma}\lambda_1'P_0 + \frac{C_n}{n - k_n\sigma}\frac{n}{\alpha + n}\sum_{i = 1}^{k_n}\frac{n_i}{n}\delta_{x_i^*}\Bigg) \\
        & \leq \lambda_1'\wtr\left( \sum_{i = 1}^{k_n}\frac{n_i - \sigma}{n - k_n\sigma}\delta_{x_i^*}, P_0\right)\\ 
        &\quad + \frac{n}{\alpha + n}\wtr \left(\sum_{i = 1}^{k_n}\frac{n_i - \sigma}{n - k_n \sigma}\delta_{x_i^*}, \sum_{i = 1}^{k_n}\frac{n_i}{n}\delta_{x_i^*} \right).
    \end{align*}
    Since $\lambda_1' \leq \frac{\alpha}{\alpha + n}$, the first term is asymptotically bounded by $\frac{k_n}{n}$. As we have already bounded the second Wasserstein distance, we deduce that $\E \mathcal{A}^{U_n} \mathbf{1}_{A_n^c} \lesssim \frac{k_n}{n}.$
\end{proof}
