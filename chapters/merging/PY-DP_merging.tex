...
\end{proof}
To summarize, differently from the caes of generalized gamma CRM vs Gamma CRM, here there is no dependency on discount parameter for merging. What matters is the rate at which the number of unique observations grow with respect to the total number of observations. If they growh with the same rate, then the two posteriors do not merge, otherwise othey merge with the rate $\frac{k_n}{n}.$