...
In practice, since we work with the hierarchical samples $\mathbf{X}_{n,m}, \mathbf{Y}_{n,m}$, permutation is implemented by shuffling rows across the two matrices. The resulting procedure is summarized in Algorithm~\ref{algorithm::permutation_approach}.
...
For the distance $\dlip$ in dimension $1$, we are able to establish theoretical guarantees for the permutation test described above. The reason is that the class of measurable functions $\dstar$ used to express the distance as an IPM in this case is a Donsker class. This implies that we can control the asymptotic behavior of both the test statistic and the permutation threshold towards norms of Brownian bridges.
The permutation test can be implemented for both metrics and in any dimension, but the asymptotic guarantees are only available for the HIPM metric in dimension $1$. 
...
\begin{itemize}
    \item if $Q^1 = Q^2,$ then $$ \limsup_{n,m\to \infty} \mathbb{P}\left(T_{n,m}>r(n,m, \theta)\right) = \theta.$$
