...
\]
Note that given the hierarchical samples $\mathbf{X}_{n,m}, Y_{n,m}$,
\[
...
\]
Let $U_1,\dots,U_{n_q}$ be drawn from $C_q$ without replacement, and let $V_1,\dots,V_{n_q}$ be drawn from $C_q$ with replacement. Then, by Proposition~A.1.10 in~\cite{van1996weak}, we have
\[
...
\]
where $\mu_1, \mu_2$ and $\sigma_1^2, \sigma_2^2$ are the means and variances of $P^1$ and $P^2$, respectively. In this setting, Fr\'{e}chet mean and variance for the law of a random probability measure $Q \in \mathcal{P}(\mathcal{P}_\N)$ can be computed from the following lemma:

\begin{lemma}\label{lemma::frechet_mean_variance}
Let $Q \in \mathcal{P}(\mathcal{P}_\N)$ where $\mathcal{P}_\N$ is as $\ref{equation:class_of_normals}$. Let $\widetilde{P} \sim Q$, and define the random mean and variance as
\[
...
\]
are Fr\'{e}chet mean and variance respectively. 
\end{lemma}
\begin{proof}
