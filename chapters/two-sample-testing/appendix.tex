...
\]
the HIPM distance is unchanged and $\dstar$ is uniformly bounded by $K.$
...
Given $Q \in \probprobx$ and the hierarchical empirical estimators $\widehat{Q}_{n,m}$ (see Definition~\ref{definition:hier_emp_estimator}), we define a sequence of empirical processes $G_{n,m,Q}$, for each $m \geq 1$, by
\[
...
\begin{align*}
\sup_{h \in \dstar} \left|G_{n,m}(h)\right|  & = \sqrt{n} \sup_{h \in \dstar} \left| \widehat{Q}_{n,m}(h) - \T_m[Q](h)\right| \\
& \leq  \sqrt{n} \sup_{h \in \dstar} \widehat{Q}_{n,m}(|h|) + \sqrt{n} \sup_{h \in \dstar} \T_m[Q](|h|) \\
& \leq 2K\sqrt{n},
\end{align*}
...
Recall the definition of the permuted hierarchical empirical estimators $\widetilde{Q}^1_{n,m}$ and $\widetilde{Q}^2_{n,m}$~\eqref{definition:permuted_hierarchical_estimators}. We also define
\[
...
\]
for $t_1,\dots,t_k \in \R.$ Therefore, it suffices to prove the latter.
...
\]
for $i = 1, \dots, N$. Then $\|u_i - l_i\|_{\mathrm{L}_2(P)} = \epsilon$ for each $i$.
...
\]
for all $y \in \Y$. Thus, $h \in [l_i, u_i]$, and the collection $\{[l_i, u_i]\}_{i=1}^N$
...
\begin{lemma}\label{lemma::covnumbdstar}
    ...
    \[
    \log \N\left(\epsilon, \dstar, \|\cdot\|_\infty\right) \leq C_d \frac{1}{\epsilon^d},
    \]
    for all $\epsilon > 0$.
\end{lemma}
\begin{proof}
Let $K = \mathrm{diam}(\X)$. As shown on page 17 of \cite{catalano2024hierarchical}, we have
\[
